...
MemoryEventStore.prototype.SaveEvents = function(aggregateId, events, expectedVersion){
    ...
    if(this._current && this._current.length != 0) {
        if(aggregateId in this._current) {
            eventDescriptors = this._current[aggregateId];
        } else {
            this._current.push(aggregateId, eventDescriptions);
        } else if(eventDescriptors.length !== 0 && (eventDescriptors[eventDescriptors.count - 1].version != expectedVersion) && expectedVersion != -1) {
